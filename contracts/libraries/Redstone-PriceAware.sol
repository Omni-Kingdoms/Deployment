...
abstract contract PriceAware {
    ...
    function isTimestampValid(uint256 _receivedTimestamp) internal view virtual returns (bool) {
        ...
        return block.timestamp < _receivedTimestamp || block.timestamp - _receivedTimestamp < getMaxDataTimestampDelay();
    }
    ...
    function _readFromCallData(bytes32[] memory symbols, uint256 dataSize, uint16 messageLength)
        private
        pure
